...
pub fn package_to_scene_from_uasset(
    ...
    // Map class names
    let class_of: Vec<String> = uasset_header.exports.iter().map(|export| {
    ...
    // Scan exports with geometry-related classes for raw byte patterns that decode as valid 3D vertices
    let geometry_classes = [
        "CubeBuilder", "BrushComponent", "StaticMeshComponent", "Polys", "Model",
        "SkeletalMeshComponent", "CapsuleComponent", "DrawSphereComponent",
    ];

    for (ei, export) in uasset_header.exports.iter().enumerate() {
        let class_name = &class_of[ei];
        if !geometry_classes.contains(&class_name.as_str()) { continue; }
        if export.serial_size < 16 { continue; }
        if export.serial_offset < 0 { continue; }
        ...
        // Scan for valid 3×f32 vertex runs at 4-byte aligned offsets
        ...
        for start in (0..sz - 12).step_by(4) {
            ...
            if !ok || verts.len() < 3 { continue; }
            // Filter: require at least 2 axes with spread > 1.0 and at least 4 vertices
            ...
            if axes_ok.iter().filter(|&&x| x).count() >= 2 && verts.len() >= 4 {
                best = verts;
                break;
            }
        }
        ...
        for v in &best {
            mesh.vertices.push(solid_rs::geometry::Vertex::new(*v));
        }
