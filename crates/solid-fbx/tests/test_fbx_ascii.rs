...
fn ascii_triangle_positions_survive_round_trip() {
    ...
    for (ov, lv) in orig.vertices.iter().zip(got.vertices.iter()) {
        assert!(
            (ov.position.x - lv.position.x).abs() < 1e-4,
            "position.x mismatch"
        );
        assert!(
            (ov.position.y - lv.position.y).abs() < 1e-4,
            "position.y mismatch"
        );
        assert!(
            (ov.position.z - lv.position.z).abs() < 1e-4,
            "position.z mismatch"
        );
    }
}
...
fn ascii_uvs_survive_round_trip() {
    ...
    mesh.vertices = vec![
        Vertex::new(Vec3::new(0.0, 1.0, 0.0))
            .with_normal(Vec3::Z)
            .with_uv(Vec2::new(0.25, 0.5)),
        Vertex::new(Vec3::new(-1.0, -1.0, 0.0))
            .with_normal(Vec3::Z)
            .with_uv(Vec2::new(0.0, 0.5)),
        Vertex::new(Vec3::new(1.0, -1.0, 0.0))
            .with_normal(Vec3::Z)
            .with_uv(Vec2::new(0.75, 0.5)),
    ];
    ...
    let orig_uvs: Vec<Vec2> = scene.meshes[0]
        .vertices
        .iter()
        .map(|v| v.uv().unwrap())
        .collect();
    let loaded_uvs: Vec<Vec2> = loaded_mesh
        .vertices
        .iter()
        .map(|v| v.uv().unwrap())
        .collect();
...
fn ascii_vertex_colors_survive_round_trip() {
    ...
    let orig_colors: Vec<Vec4> = scene.meshes[0]
        .vertices
        .iter()
...
fn ascii_tangents_survive_round_trip() {
    ...
    let loaded = ascii_round_trip(&scene);
    assert!(!loaded.meshes.is_empty());
    let has_tangents = loaded.meshes[0]
        .vertices
        .iter()
        .any(|v| v.tangent.is_some());
...
fn ascii_material_diffuse_survives() {
    ...
    let loaded = ascii_round_trip(&scene);
    assert!(
        !loaded.materials.is_empty(),
        "no materials after round-trip"
    );
    let orig = &scene.materials[0];
    let got = &loaded.materials[0];
    assert!(
        (got.base_color_factor.x - orig.base_color_factor.x).abs() < 1e-3,
        "diffuse.r"
    );
    assert!(
        (got.base_color_factor.y - orig.base_color_factor.y).abs() < 1e-3,
        "diffuse.g"
    );
    assert!(
        (got.base_color_factor.z - orig.base_color_factor.z).abs() < 1e-3,
        "diffuse.b"
    );
}
...
fn ascii_material_emissive_survives() {
    ...
    let got = &loaded.materials[0];
    assert!(
        (got.emissive_factor.x - orig.emissive_factor.x).abs() < 1e-3,
        "emissive.r"
    );
    assert!(
        (got.emissive_factor.y - orig.emissive_factor.y).abs() < 1e-3,
        "emissive.g"
    );
    assert!(
        (got.emissive_factor.z - orig.emissive_factor.z).abs() < 1e-3,
        "emissive.b"
    );
}
...
fn ascii_diffuse_texture_survives() {
    ...
    assert!(
        uri.contains("diffuse"),
        "diffuse URI not preserved: {:?}",
        uri
    );
}
...
fn ascii_node_translation_survives() {
    ...
    b.set_transform(
        child,
        Transform::IDENTITY.with_translation(Vec3::new(1.5, 2.5, -3.5)),
    );
    let scene = b.build();
    let loaded = ascii_round_trip(&scene);
    ...
    let node = loaded
        .nodes
        .iter()
        .find(|n| n.name == "Translated")
        .expect("Translated missing");
...
fn ascii_node_rotation_survives() {
    ...
    let node = loaded
        .nodes
        .iter()
        .find(|n| n.name == "Rotated")
        .expect("Rotated missing");
    ...
    assert!(
        (expected.x - actual.x).abs() < 1e-2,
        "rot.x {} vs {}",
        expected.x,
        actual.x
    );
    assert!(
        (expected.y - actual.y).abs() < 1e-2,
        "rot.y {} vs {}",
        expected.y,
        actual.y
    );
    assert!(
        (expected.z - actual.z).abs() < 1e-2,
        "rot.z {} vs {}",
        expected.z,
        actual.z
    );
...
fn ascii_node_scale_survives() {
    ...
    let node = loaded
        .nodes
        .iter()
        .find(|n| n.name == "Scaled")
        .expect("Scaled missing");
    let s = node.transform.scale;
    assert!(
        (s.x - orig_scale.x).abs() < 1e-3,
        "scale.x {} vs {}",
        s.x,
        orig_scale.x
    );
    assert!(
        (s.y - orig_scale.y).abs() < 1e-3,
        "scale.y {} vs {}",
        s.y,
        orig_scale.y
    );
    assert!(
        (s.z - orig_scale.z).abs() < 1e-3,
        "scale.z {} vs {}",
        s.z,
        orig_scale.z
    );
...
fn ascii_point_light_survives() {
    ...
    assert!(
        (pt.unwrap().intensity() - 100.0).abs() < 1.0,
        "point light intensity off"
    );
}
...
fn ascii_directional_light_survives() {
    ...
    assert!(
        loaded
            .lights
            .iter()
            .any(|l| matches!(l, Light::Directional(_))),
...
fn ascii_skin_weights_survive_round_trip() {
    ...
    if let Some(v) = mesh.vertices.iter().find(|v| v.position.length() < 1e-3) {
        if let Some(sw) = &v.skin_weights {
            assert!(
                sw.weights[0] > 0.9,
                "joint-0 weight should be ~1.0, got {}",
                sw.weights[0]
            );
        }
    }
...
fn ascii_animation_translation_survives() {
    ...
    assert!(
        ...
        "no animations after round-trip"
    );
    let ch = loaded.animations[0]
        .channels
        .iter()
    ...
    assert!(
        (ch.values[n - 3] - 1.0).abs() < 1e-2,
        "T.x at end: {}",
        ch.values[n - 3]
    );
    assert!(
        (ch.values[n - 2] - 0.0).abs() < 1e-2,
        "T.y at end: {}",
        ch.values[n - 2]
    );
    assert!(
        (ch.values[n - 1] - 0.0).abs() < 1e-2,
        "T.z at end: {}",
        ch.values[n - 1]
    );
}
...
fn ascii_animation_scale_survives() {
    ...
    assert!(
        (ch.values[n - 3] - 2.0).abs() < 1e-2,
        "S.x at end: {}",
        ch.values[n - 3]
    );
    assert!(
        (ch.values[n - 2] - 2.0).abs() < 1e-2,
        "S.y at end: {}",
        ch.values[n - 2]
    );
    assert!(
        (ch.values[n - 1] - 2.0).abs() < 1e-2,
        "S.z at end: {}",
        ch.values[n - 1]
    );
}
...
fn ascii_per_polygon_material_survives() {
    ...
    assert!(
        loaded.materials.len() >= 2,
        "need ≥2 materials, got {}",
    ...
    assert_ne!(
        p0.material_index, p1.material_index,
...
fn ascii_empty_scene_round_trips() {
    ...
    assert!(
        ...
        "expected no meshes in empty scene"
    );
    assert!(
        loaded.materials.is_empty(),
        "expected no materials in empty scene"
    );
}
