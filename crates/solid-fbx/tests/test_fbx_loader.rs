...
fn loader_accepts_binary_produced_by_saver() {
    ...
    assert!(
        result.is_ok(),
        "loader rejected binary FBX produced by saver: {:?}",
        result.err()
    );
}
...
fn loader_accepts_ascii_produced_by_saver() {
    ...
    assert!(
        result.is_ok(),
        "loader rejected ASCII FBX produced by saver: {:?}",
        result.err()
    );
}
...
fn loader_empty_scene_has_no_meshes() {
    ...
    assert!(
        ...
        "expected 0 meshes from empty scene"
    );
    assert!(
        loaded.materials.is_empty(),
        "expected 0 materials from empty scene"
    );
    assert!(
        loaded.lights.is_empty(),
        "expected 0 lights from empty scene"
    );
    assert!(
        loaded.cameras.is_empty(),
        "expected 0 cameras from empty scene"
    );
}
...
fn loader_triangle_has_one_primitive() {
    ...
    assert!(!loaded.meshes.is_empty(), "no meshes");
    assert_eq!(
        loaded.meshes[0].primitives.len(),
        1,
        "triangle should have 1 primitive, got {}",
        loaded.meshes[0].primitives.len()
    );
}
