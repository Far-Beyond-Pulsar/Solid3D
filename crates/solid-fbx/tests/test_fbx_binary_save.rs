...
fn binary_save_produces_nonzero_bytes() {
    ...
    assert!(
        buf.len() > 100,
        "binary output suspiciously small: {} bytes",
        buf.len()
    );
}
...
fn binary_save_is_parseable_by_loader() {
    ...
    assert!(
        result.is_ok(),
        "loader failed on binary output: {:?}",
        result.err()
    );
}
...
fn binary_save_empty_scene() {
    ...
    FbxSaver.save_binary(&scene, &mut buf).unwrap();
    assert!(
        &buf[..23] == FBX_MAGIC.as_slice(),
        "magic missing for empty scene"
    );
    ...
    assert!(
        ...
        "empty scene should have no meshes"
    );
}
...
fn binary_round_trip_mesh_name() {
    ...
    assert!(!loaded.meshes.is_empty());
    assert_eq!(
        loaded.meshes[0].name, "NamedMesh",
        "mesh name not preserved"
    );
}
...
fn binary_save_and_ascii_save_same_mesh_count() {
    ...
    assert_eq!(
        loaded_ascii.meshes.len(),
        loaded_binary.meshes.len(),
