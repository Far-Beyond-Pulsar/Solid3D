...
impl<'d> Converter<'d> {
    ...
    fn run(&mut self) -> Result<Scene> {
            ...
            if let (Some(&ti), Some(&mi)) = (self.tex_fbx.get(src_id), self.mat_fbx.get(dst_id)) {
                match prop.as_str() {
                    "DiffuseColor" | "Diffuse" => {
                        mat_diffuse_tex.insert(mi, ti);
                    }
                    "NormalMap" | "Bump" => {
                        mat_normal_tex.insert(mi, ti);
                    }
                    _ => {}
                }
            }
        ...
        let geom_to_model: HashMap<i64, i64> = model_to_geom
            .iter()
        ...
        let geom_to_skin: HashMap<i64, i64> =
            skin_to_geom.iter().map(|(&sid, &gid)| (gid, sid)).collect();

        // Apply skin weights to expanded vertices
        for gi in 0..self.geoms.len() {
            let geom_fbx_id = self.geoms[gi].fbx_id;
            let skin_id = match geom_to_skin.get(&geom_fbx_id) {
                Some(&s) => s,
                None => continue,
            };
            ...
            for (ji, &cluster_id) in cluster_ids.iter().enumerate() {
                let ci = match self.cluster_fbx.get(&cluster_id) {
                    Some(&i) => i,
                    None => continue,
                };
            ...
            for (vi_exp, vi_orig) in orig_indices.iter().enumerate() {
                if *vi_orig >= vert_influences.len() {
                    continue;
                }
                ...
                self.geoms[gi].vertices[vi_exp].skin_weights =
        ...
        for (ri, raw) in self.geoms.iter().enumerate() {
            ...
            if raw.mat_mapping_all_same || raw.poly_mat_indices.is_empty() {
                ...
                let local_to_scene: Vec<Option<usize>> = model_mats
                    .iter()
                    .map(|fbx_mid| {
                        self.mat_fbx
                            .get(fbx_mid)
        ...
        let mut queue: Vec<i64> = model_fbx_ids
            .iter()
            .filter(|id| !model_to_parent.contains_key(*id))
            .cloned()
            .collect();
        let mut remaining: Vec<i64> = model_fbx_ids
            .iter()
            .filter(|id| model_to_parent.contains_key(*id))
            .cloned()
            .collect();
        ...
        loop {
            ...
            if queue.is_empty() && remaining.is_empty() {
                break;
            }
        ...
        for si in 0..self.skins.len() {
            ...
            let geom_fbx_id = match skin_to_geom.get(&skin_fbx_id) {
                Some(&g) => g,
                None => continue,
            };
            let cluster_ids: Vec<i64> = skin_to_clusters
                .get(&skin_fbx_id)
                .cloned()
                .unwrap_or_default();
            ...
            for &cluster_id in &cluster_ids {
                let joint_model_fbx_id = match cluster_to_joint.get(&cluster_id) {
                    Some(&j) => j,
                    None => continue,
                };
                let joint_node_id = match created_nodes.get(&joint_model_fbx_id) {
                    Some(&nid) => nid,
                    None => continue,
                };
                let ci = match self.cluster_fbx.get(&cluster_id) {
                ...
                skin.inverse_bind_matrices
                    .push(self.clusters[ci].transform_link.inverse());
            }
        ...
        for &(src_id, dst_id) in &self.oo_conns {
            if self.anim_layer_fbx.contains_key(&src_id)
                && self.anim_stack_fbx.contains_key(&dst_id)
            {
                layer_to_stack.insert(src_id, dst_id);
            } else if self.curve_node_fbx.contains_key(&src_id)
                && self.anim_layer_fbx.contains_key(&dst_id)
            {
        ...
        for (src_id, dst_id, prop) in &self.op_conns {
            ...
            } else if self.curve_fbx.contains_key(src_id)
                && self.curve_node_fbx.contains_key(dst_id)
            {
                match prop.as_str() {
                    "d|X" | "d|Y" | "d|Z" => {
        ...
        for si in 0..self.anim_stacks.len() {
            ...
            let curve_node_ids: Vec<i64> = stack_to_curve_nodes
                .get(&stack_fbx_id)
                .unwrap_or(&empty_vec)
                .clone();
            ...
            for curve_node_id in curve_node_ids {
                ...
                let node_id = match created_nodes.get(&model_fbx_id) {
                    Some(&nid) => nid,
                    None => continue,
                };
                ...
                if tx.is_empty() && ty.is_empty() && tz.is_empty() {
                    continue;
                }
                ...
                match pname {
                    ...
                    "Shininess" | "ShininessExponent" => {
                        ...
                        mat.roughness_factor =
                            ((2.0 / (s as f64 + 2.0)).sqrt() as f32).clamp(0.0, 1.0);
                    }
    ...
    fn extract_light(&mut self, id: i64, node: &FbxNode) {
        ...
        let light = match light_type {
            1 => Light::Directional(DirectionalLight {
                base,
                extensions: Extensions::new(),
            }),
        ...
        match sub_type {
            ...
            "Cluster" => {
                ...
                let tl_vec = node
                    ...
                    .map(|n| n.as_f64_vec())
                    .unwrap_or_default();
                let transform_link: Mat4 = if tl_vec.is_empty() {
                    Mat4::IDENTITY
                } else {
                    mat4_from_cols(&tl_vec)
                };

                let idx = self.clusters.len();
                self.cluster_fbx.insert(id, idx);
                self.clusters.push(RawCluster {
                    fbx_id: id,
                    indexes,
                    weights,
                    transform_link,
