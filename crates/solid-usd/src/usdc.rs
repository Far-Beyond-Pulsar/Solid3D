...
pub fn read(data: &[u8]) -> Result<UsdDoc, SolidError> {
    ...
    let toc_offset =
        u64::from_le_bytes(data[TOC_OFFSET_POS..TOC_OFFSET_POS + 8].try_into().unwrap()) as usize;
...
fn read_specs_section(sec: &[u8]) -> Result<Vec<(u32, u32, u32)>, SolidError> {
    ...
    Ok((0..num)
        .map(|i| (path_idxs[i], fieldset_idxs[i], spec_types[i]))
        .collect())
}
        ...
        match spec_type {
            ...
            SPEC_RELATIONSHIP => {
                if let Some(dot) = path.rfind('.') {
                    let parent = path[..dot].to_string();
                    let name = path[dot + 1..].to_string();
                    let rel = decode_relationship_spec(
                        &name,
                        fs_idx as usize,
                        fieldsets,
                        fields,
    ...
    for &(path_idx, fs_idx, spec_type) in specs {
        ...
        match spec_type {
            ...
            SPEC_PRIM => {
                ...
                let (specifier, type_name) =
                    decode_prim_fields(fs_idx as usize, fieldsets, fields, tokens);
...
fn decode_prim_fields(
    ...
    let set = match fieldsets.get(fs_idx) {
        Some(s) => s,
        None => return (specifier, type_name),
    };
    for &fi in set {
        let (name_tok, vrep) = match fields.get(fi as usize) {
            Some(f) => *f,
            None => continue,
        };
        let key = tokens
            .get(name_tok as usize)
            .map(|s| s.as_str())
            .unwrap_or("");
        match key {
            "specifier" => {
...
fn inline_prim_attrs(
    ...
    for &fi in set {
        ...
        if matches!(
            key,
            "specifier" | "typeName" | "properties" | "primChildren"
        ) {
            continue;
        }
        ...
        match key {
            "typeName" => {
                if (vrep & VR_IS_INLINED) != 0 {
                    ...
                    if ty_code == TY_TOKEN {
                        type_name = tokens
                            .get((vrep & VR_PAYLOAD_MASK) as usize)
                            .cloned()
                            .unwrap_or_default();
                    }
                }
...
fn decode_inline_value(
    ...
    match ty_code {
        ...
        TY_TOKEN => Some(UsdValue::Token(
            tokens.get(payload as usize).cloned().unwrap_or_default(),
        )),
        TY_STRING => Some(UsdValue::String(
            strings.get(payload as usize).cloned().unwrap_or_default(),
        )),
        TY_ASSET => Some(UsdValue::Asset(
            tokens.get(payload as usize).cloned().unwrap_or_default(),
        )),
...
fn decode_scalar_at(
    ...
    match ty_code {
        TY_DOUBLE => {
            ...
            Some(UsdValue::Float(f64::from_le_bytes(
                d[offset..offset + 8].try_into().ok()?,
            )))
        }
        TY_FLOAT => {
            if offset + 4 > d.len() {
            ...
            Some(UsdValue::Float(f32::from_bits(u32::from_le_bytes(
                d[offset..offset + 4].try_into().ok()?,
            )) as f64))
        }
        TY_VEC2F | TY_VEC2D => {
            if offset + 8 > d.len() {
            ...
            let b = f32::from_bits(u32::from_le_bytes(
                d[offset + 4..offset + 8].try_into().ok()?,
            ));
        ...
        TY_VEC3F | TY_VEC3D | TY_VEC3I => {
            ...
            let z = f32::from_bits(u32::from_le_bytes(
                d[offset + 8..offset + 12].try_into().ok()?,
            ));
        ...
        TY_VEC4F | TY_QUATF => {
            ...
            let w = f32::from_bits(u32::from_le_bytes(
                d[offset + 12..offset + 16].try_into().ok()?,
            ));
        ...
        TY_TOKEN | TY_ASSET => {
            ...
            if ty_code == TY_ASSET {
                Some(UsdValue::Asset(s))
            } else {
                Some(UsdValue::Token(s))
            }
        }
        _ => None,
    }
...
fn lz4_decompress(src: &[u8], expected: usize) -> Result<Vec<u8>, SolidError> {
    ...
    for skip in 1..=16 {
        ...
        if let Ok(out) = lz4_flex::block::decompress(&src[skip..], expected) {
            println!(
                "USDC lz4 decompress: succeeded after skipping {} bytes of padding",
                skip
            );
            return Ok(out);
        }
    ...
    if src.len() == expected {
        println!(
            "USDC lz4 decompress: expected compressed data but got raw bytes; returning raw slice"
        );
        return Ok(src.to_vec());
    }

    Err(SolidError::parse(
        "USDC lz4 decompress: data is not a valid LZ4 block/frame/size-prefixed block",
    ))
}
...
fn ru8(d: &[u8], pos: &mut usize) -> Result<u8, SolidError> {
    let v = *d
        .get(*pos)
        .ok_or_else(|| SolidError::parse("USDC: unexpected EOF (u8)"))?;
    *pos += 1;
    Ok(v)
}
...
fn ru16(d: &[u8], pos: &mut usize) -> Result<u16, SolidError> {
    let s = d
        .get(*pos..*pos + 2)
        .ok_or_else(|| SolidError::parse("USDC: unexpected EOF (u16)"))?;
    *pos += 2;
...
fn ru32(d: &[u8], pos: &mut usize) -> Result<u32, SolidError> {
    let s = d
        .get(*pos..*pos + 4)
        .ok_or_else(|| SolidError::parse("USDC: unexpected EOF (u32)"))?;
    *pos += 4;
...
fn ri64(d: &[u8], pos: &mut usize) -> Result<i64, SolidError> {
    let s = d
        .get(*pos..*pos + 8)
        .ok_or_else(|| SolidError::parse("USDC: unexpected EOF (i64)"))?;
    *pos += 8;
...
fn rbytes<'a>(d: &'a [u8], pos: &mut usize, n: usize) -> Result<&'a [u8], SolidError> {
    let s = d
        .get(*pos..*pos + n)
        .ok_or_else(|| SolidError::parse(format!("USDC: unexpected EOF ({n} bytes)")))?;
    *pos += n;
    Ok(s)
}
...
#[allow(dead_code)]
fn _ru8_unused(d: &[u8], p: &mut usize) -> Result<u8, SolidError> {
    ru8(d, p)
}
#[allow(dead_code)]
fn _ru16_unused(d: &[u8], p: &mut usize) -> Result<u16, SolidError> {
    ru16(d, p)
}
